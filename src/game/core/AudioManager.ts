...
export class AudioManager {
  ...
  initialize(_scene: Scene): void {
    // scene might be used in future for spatial sound
  }

  // Play a sound effect
  play(effect: SoundEffect, options?: { volume?: number; position?: Vector3 }): void {
    if (this.isMuted || !this.proceduralAudio.isAvailable) return;
  ...
  // Stop all loops
  stopAllLoops(): void {
    for (const [_key, loop] of this.activeLoops) {
      loop.stop();
    }
