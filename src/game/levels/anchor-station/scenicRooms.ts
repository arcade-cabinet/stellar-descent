...
async function preloadModels(scene: Scene, paths: string[]): Promise<void> {
  ...
  await Promise.allSettled(
    unique.map((p) => AssetManager.loadAssetByPath(p, scene))
  );
}
...
async function createObservationDeck(
  ...
  await preloadModels(scene, [
    P.floorCeiling1, P.modRoofVents, P.modRoofPlate, P.wallSingle, P.wallDouble,
    P.doorway2, P.window1, P.modColumn1, P.beamHorizontal1, P.pipe1,
    P.bench, P.modVesselShort, P.modVessel, P.lamp1, P.lamp2,
    P.modVent1, P.modVent2, P.modPlateLarge,
    P.npcMarine, P.npcMarineSergeant,
  ]);

  // --- Floor tiles (4m grid coverage for 16x10 room) ---
  for (let fx = -6; fx <= 6; fx += 4) {
    for (let fz = -4; fz <= 4; fz += 4) {
      placeModel(scene, P.floorCeiling1, `od_floor_${fx}_${fz}`, room,
        OC.x + fx, -0.05, OC.z + fz, 0, 1, allMeshes);
    }
  }
  ...
  for (let cx = -6; cx <= 6; cx += 4) {
    for (let cz = -4; cz <= 4; cz += 4) {
      ...
      placeModel(scene, isVent ? P.modRoofVents : P.modRoofPlate,
        `od_ceil_${cx}_${cz}`, room,
        OC.x + cx, OBSERVATION_DECK.height, OC.z + cz, 0, 1, allMeshes);
    }
  }
  ...
  for (let wz = -4; wz <= 4; wz += 4) {
    placeModel(scene, P.wallSingle, `od_wall_l_${wz}`, room,
      OC.x - OBSERVATION_DECK.width / 2, 0, OC.z + wz, Math.PI / 2, 1, allMeshes);
    placeModel(scene, P.wallSingle, `od_wall_r_${wz}`, room,
      OC.x + OBSERVATION_DECK.width / 2, 0, OC.z + wz, -Math.PI / 2, 1, allMeshes);
  }

  // Back wall (with entrance from scenic corridor)
  placeModel(scene, P.wallDouble, 'od_wall_back_l', room,
    OC.x - 5, 0, OC.z - OBSERVATION_DECK.depth / 2, Math.PI, 1, allMeshes);
  placeModel(scene, P.wallDouble, 'od_wall_back_r', room,
    OC.x + 5, 0, OC.z - OBSERVATION_DECK.depth / 2, Math.PI, 1, allMeshes);
  // Doorway frame at back entrance
  placeModel(scene, P.doorway2, 'od_door_back', room,
    OC.x, 0, OC.z - OBSERVATION_DECK.depth / 2, Math.PI, 1, allMeshes);

  // --- Windows along front wall (observation windows) ---
  for (let wx = -4; wx <= 4; wx += 4) {
    placeModel(scene, P.window1, `od_win_front_${wx}`, room,
      OC.x + wx, 0.5, OC.z + OBSERVATION_DECK.depth / 2 - 0.2, 0, 1, allMeshes);
  }

  // --- Window frame wall sections alongside the windows ---
  placeModel(scene, P.wallSingle, 'od_wall_front_l', room,
    OC.x - 6, 0, OC.z + OBSERVATION_DECK.depth / 2, 0, 1, allMeshes);
  placeModel(scene, P.wallSingle, 'od_wall_front_r', room,
    OC.x + 6, 0, OC.z + OBSERVATION_DECK.depth / 2, 0, 1, allMeshes);

  // --- Pillars at room corners ---
  placeProp(scene, P.modColumn1, 'od_col_nw', room,
    OC.x - 7, 0, OC.z + 4, 0, 1, allMeshes);
  placeProp(scene, P.modColumn1, 'od_col_ne', room,
    OC.x + 7, 0, OC.z + 4, 0, 1, allMeshes);
  placeProp(scene, P.modColumn1, 'od_col_sw', room,
    OC.x - 7, 0, OC.z - 4, 0, 1, allMeshes);
  placeProp(scene, P.modColumn1, 'od_col_se', room,
    OC.x + 7, 0, OC.z - 4, 0, 1, allMeshes);

  // --- Ceiling beams ---
  placeProp(scene, P.beamHorizontal1, 'od_beam_1', room,
    OC.x, OBSERVATION_DECK.height - 0.3, OC.z, 0, 1, allMeshes);

  // --- Ceiling pipes ---
  placeProp(scene, P.pipe1, 'od_pipe_1', room,
    OC.x - 3, OBSERVATION_DECK.height - 0.15, OC.z, 0, 1, allMeshes);

  // --- Seating: benches for couches ---
  placeProp(scene, P.bench, 'od_bench_1', room,
    OC.x - 4, 0, OC.z + 1.5, 0, 1, allMeshes);
  placeProp(scene, P.bench, 'od_bench_2', room,
    OC.x, 0, OC.z + 1.5, 0, 1, allMeshes);
  placeProp(scene, P.bench, 'od_bench_3', room,
    OC.x + 4, 0, OC.z + 1.5, 0, 1, allMeshes);
  // Side chairs
  placeProp(scene, P.bench, 'od_chair_l', room,
    OC.x - 5, 0, OC.z - 1, 0.3, 0.7, allMeshes);
  placeProp(scene, P.bench, 'od_chair_r', room,
    OC.x + 5, 0, OC.z - 1, -0.3, 0.7, allMeshes);

  // --- Plants: vessels as planters ---
  placeProp(scene, P.modVesselShort, 'od_plant_1', room,
    OC.x - 6, 0, OC.z + 3, 0, 1.2, allMeshes);
  placeProp(scene, P.modVesselShort, 'od_plant_2', room,
    OC.x + 6, 0, OC.z + 3, 0, 1, allMeshes);
  placeProp(scene, P.modVessel, 'od_plant_3', room,
    OC.x - 3, 0, OC.z - 3, 0, 0.8, allMeshes);
  placeProp(scene, P.modVessel, 'od_plant_4', room,
    OC.x + 3, 0, OC.z - 3, 0, 0.9, allMeshes);

  // --- Lamps on ceiling ---
  placeProp(scene, P.lamp1, 'od_lamp_1', room,
    OC.x - 4, OBSERVATION_DECK.height - 0.3, OC.z, 0, 1, allMeshes);
  placeProp(scene, P.lamp2, 'od_lamp_2', room,
    OC.x + 4, OBSERVATION_DECK.height - 0.3, OC.z, 0, 1, allMeshes);

  // --- Wall vents ---
  placeProp(scene, P.modVent1, 'od_vent_1', room,
    OC.x - 7.5, 3.2, OC.z, Math.PI / 2, 1, allMeshes);
  placeProp(scene, P.modVent2, 'od_vent_2', room,
    OC.x + 7.5, 3.2, OC.z, -Math.PI / 2, 1, allMeshes);

  // --- Wall detail plates ---
  placeProp(scene, P.modPlateLarge, 'od_plate_1', room,
    OC.x - 3, OBSERVATION_DECK.height - 0.1, OC.z + 2, 0, 1, allMeshes);
  ...
  planetSphere.position = new Vector3(
    OC.x + 5,
    -8,
    OC.z + OBSERVATION_DECK.depth / 2 + 40
  );
  ...
  starsBackdrop.position = new Vector3(
    OC.x,
    10,
    OC.z + OBSERVATION_DECK.depth / 2 + 80
  );
  ...
  addCeilingLight(
    ...
    new Color3(1.0, 0.95, 0.85), 0.4, lights
  );
  addCeilingLight(
    scene, room,
    new Vector3(OC.x + 4, OBSERVATION_DECK.height - 0.2, OC.z),
  ...
  addCeilingLight(
    ...
    new Color3(0.9, 0.92, 1.0), 0.35, lights
  );
...
async function createMessHall(
  ...
  for (let fx = -6; fx <= 6; fx += 4) {
    for (let fz = -4; fz <= 4; fz += 4) {
      placedNodes.push(
        placeModel(scene, P.floorCeiling1, `mh_floor_${fx}_${fz}`, room,
          MC.x + fx, -0.05, MC.z + fz, 0, 1, allMeshes)
      );
    }
  }
  ...
  for (let cx = -6; cx <= 6; cx += 4) {
    for (let cz = -4; cz <= 4; cz += 4) {
      placedNodes.push(
        placeModel(scene, P.modRoofPlate, `mh_ceil_${cx}_${cz}`, room,
          MC.x + cx, MESS_HALL.height, MC.z + cz, 0, 1, allMeshes)
      );
    }
  }
  ...
  for (let wx = -6; wx <= 6; wx += 4) {
    placedNodes.push(
      placeModel(scene, P.wallDouble, `mh_wall_back_${wx}`, room,
        MC.x + wx, 0, MC.z + MESS_HALL.depth / 2, 0, 1, allMeshes)
    );
  }
  // Front wall (with door opening to corridor)
  placedNodes.push(
    placeModel(scene, P.wallDouble, 'mh_wall_front_l', room,
      MC.x - 4, 0, MC.z - MESS_HALL.depth / 2, Math.PI, 1, allMeshes)
  );
  placedNodes.push(
    placeModel(scene, P.wallDouble, 'mh_wall_front_r', room,
      MC.x + 4, 0, MC.z - MESS_HALL.depth / 2, Math.PI, 1, allMeshes)
  );
  placedNodes.push(
    placeModel(scene, P.doorway, 'mh_door_front', room,
      MC.x, 0, MC.z - MESS_HALL.depth / 2, Math.PI, 1, allMeshes)
  );

  // Left wall (with kitchen window opening)
  for (let wz = -4; wz <= 4; wz += 4) {
    placedNodes.push(
      placeModel(scene, P.wallSingle, `mh_wall_l_${wz}`, room,
        MC.x - MESS_HALL.width / 2, 0, MC.z + wz, Math.PI / 2, 1, allMeshes)
    );
  }
  // Window on left wall
  placedNodes.push(
    placeModel(scene, P.window1, 'mh_win_kitchen', room,
      MC.x - MESS_HALL.width / 2 + 0.2, 0.5, MC.z + 1, Math.PI / 2, 1, allMeshes)
  );
  // Right wall
  for (let wz = -4; wz <= 4; wz += 4) {
    placedNodes.push(
      placeModel(scene, P.wallSingle, `mh_wall_r_${wz}`, room,
        MC.x + MESS_HALL.width / 2, 0, MC.z + wz, -Math.PI / 2, 1, allMeshes)
    );
  }
  ...
  placedNodes.push(
    placeProp(scene, P.modCrateLong, 'mh_counter', room,
      MC.x - MESS_HALL.width / 2 - 0.6, 0, MC.z + 1, Math.PI / 2, 1, allMeshes)
  );
  ...
  for (let row = 0; row < 2; row++) {
    ...
    placedNodes.push(
      placeProp(scene, P.modCrateLong, `mh_table_${row}`, room,
        MC.x + 1, 0.5, rowZ, 0, 1.2, allMeshes)
    );
    // Benches on both sides
    placedNodes.push(
      placeProp(scene, P.bench, `mh_bench_${row}_a`, room,
        MC.x + 1, 0, rowZ - 1, 0, 1, allMeshes)
    );
    placedNodes.push(
      placeProp(scene, P.bench, `mh_bench_${row}_b`, room,
        MC.x + 1, 0, rowZ + 1, Math.PI, 1, allMeshes)
    );
  }
  ...
  for (let i = 0; i < 3; i++) {
    placedNodes.push(
      placeProp(scene, P.modShelfTall, `mh_dispenser_${i}`, room,
        MC.x + 3 + i * 1.8, 0, MC.z + MESS_HALL.depth / 2 - 0.4, 0, 1, allMeshes)
    );
  }

  // --- Computers as dispenser screens ---
  placedNodes.push(
    placeProp(scene, P.modComputerSmall, 'mh_screen_1', room,
      MC.x + 3, 0.8, MC.z + MESS_HALL.depth / 2 - 0.6, 0, 0.6, allMeshes)
  );
  placedNodes.push(
    placeProp(scene, P.modComputerSmall, 'mh_screen_2', room,
      MC.x + 4.8, 0.8, MC.z + MESS_HALL.depth / 2 - 0.6, 0, 0.6, allMeshes)
  );

  // --- Barrels for decoration ---
  placedNodes.push(
    placeProp(scene, P.barrel1, 'mh_barrel_1', room,
      MC.x - 5, 0, MC.z - 5, 0, 1, allMeshes)
  );

  // --- Wall vents ---
  placedNodes.push(
    placeProp(scene, P.modVent1, 'mh_vent_1', room,
      MC.x - 6.5, 2.8, MC.z, Math.PI / 2, 1, allMeshes)
  );
  placedNodes.push(
    placeProp(scene, P.modVent2, 'mh_vent_2', room,
      MC.x + 6.5, 2.8, MC.z, -Math.PI / 2, 1, allMeshes)
  );

  // --- Ceiling beams ---
  placedNodes.push(
    placeProp(scene, P.beamHorizontal1, 'mh_beam_1', room,
      MC.x, MESS_HALL.height - 0.3, MC.z, 0, 1, allMeshes)
  );

  // --- Lamps ---
  placedNodes.push(
    placeProp(scene, P.lamp1, 'mh_lamp_1', room,
      MC.x - 3, MESS_HALL.height - 0.3, MC.z, 0, 1, allMeshes)
  );
  placedNodes.push(
    placeProp(scene, P.lamp2, 'mh_lamp_2', room,
      MC.x + 3, MESS_HALL.height - 0.3, MC.z, 0, 1, allMeshes)
  );
  ...
  addCeilingLight(
    ...
    new Color3(1.0, 0.98, 0.95), 0.5, lights
  );
  addCeilingLight(
    scene, room,
...
async function createRecreationRoom(
  ...
  for (let fx = -4; fx <= 4; fx += 4) {
    for (let fz = -2; fz <= 2; fz += 4) {
      placedNodes.push(
        placeModel(scene, P.floorCeiling1, `rr_floor_${fx}_${fz}`, room,
          RC.x + fx, -0.05, RC.z + fz, 0, 1, allMeshes)
      );
    }
  }
  ...
  for (let cx = -4; cx <= 4; cx += 4) {
    for (let cz = -2; cz <= 2; cz += 4) {
      placedNodes.push(
        placeModel(scene, P.modRoofPlate, `rr_ceil_${cx}_${cz}`, room,
          RC.x + cx, RECREATION_ROOM.height, RC.z + cz, 0, 1, allMeshes)
      );
    }
  }
  ...
  for (let wx = -4; wx <= 4; wx += 4) {
    placedNodes.push(
      placeModel(scene, P.wallSingle, `rr_wall_back_${wx}`, room,
        RC.x + wx, 0, RC.z + RECREATION_ROOM.depth / 2, 0, 1, allMeshes)
    );
  }
  // Front wall
  for (let wx = -4; wx <= 4; wx += 4) {
    placedNodes.push(
      placeModel(scene, P.wallSingle, `rr_wall_front_${wx}`, room,
        RC.x + wx, 0, RC.z - RECREATION_ROOM.depth / 2, Math.PI, 1, allMeshes)
    );
  }
  // Left wall
  placedNodes.push(
    placeModel(scene, P.wallSingle, 'rr_wall_left', room,
      RC.x - RECREATION_ROOM.width / 2, 0, RC.z, Math.PI / 2, 1, allMeshes)
  );
  // Right wall with door opening
  placedNodes.push(
    placeModel(scene, P.wallSingle, 'rr_wall_right_top', room,
      RC.x + RECREATION_ROOM.width / 2, 0, RC.z - 2, Math.PI / 2, 1, allMeshes)
  );
  placedNodes.push(
    placeModel(scene, P.wallSingle, 'rr_wall_right_bot', room,
      RC.x + RECREATION_ROOM.width / 2, 0, RC.z + 2, Math.PI / 2, 1, allMeshes)
  );
  // Doorway frame on right wall
  placedNodes.push(
    placeModel(scene, P.doorway, 'rr_door_right', room,
      RC.x + RECREATION_ROOM.width / 2, 0, RC.z, -Math.PI / 2, 1, allMeshes)
  );

  // --- Exercise equipment: use crates/bases as treadmill and weight bench ---
  placedNodes.push(
    placeProp(scene, P.modCrate, 'rr_treadmill', room,
      RC.x - 3, 0, RC.z + 2, 0, 1, allMeshes)
  );
  placedNodes.push(
    placeProp(scene, P.modBase, 'rr_treadmill_base', room,
      RC.x - 3, 0, RC.z + 1.1, 0, 0.6, allMeshes)
  );

  // Weight bench: crate
  placedNodes.push(
    placeProp(scene, P.modCrateLong, 'rr_weight_bench', room,
      RC.x - 3, 0, RC.z - 2, 0, 0.8, allMeshes)
  );

  // Weight rack: shelf
  placedNodes.push(
    placeProp(scene, P.modShelf, 'rr_weight_rack', room,
      RC.x - 3.5, 0, RC.z - 3.5, 0, 1, allMeshes)
  );

  // --- Barrels (weights stand-in) ---
  placedNodes.push(
    placeProp(scene, P.barrel1, 'rr_barrel_1', room,
      RC.x - 4, 0, RC.z - 3.5, 0, 0.5, allMeshes)
  );
  placedNodes.push(
    placeProp(scene, P.barrel2, 'rr_barrel_2', room,
      RC.x - 3.5, 0, RC.z - 3.5, 0.3, 0.5, allMeshes)
  );

  // --- Game table: use chest as table ---
  placedNodes.push(
    placeProp(scene, P.modChest, 'rr_game_table', room,
      RC.x + 2, 0, RC.z, 0, 1, allMeshes)
  );
  ...
  for (let chair = 0; chair < 3; chair++) {
    ...
    placedNodes.push(
      placeProp(scene, P.modCrate, `rr_chair_${chair}`, room,
        RC.x + 2 + Math.cos(angle) * 1.1, 0,
        RC.z + Math.sin(angle) * 1.1, angle + Math.PI, 0.4, allMeshes)
    );
  }
  ...
  for (let l = 0; l < 4; l++) {
    placedNodes.push(
      placeProp(scene, P.modShelfTall, `rr_locker_${l}`, room,
        RC.x + 0.5 + l * 0.9, 0,
        RC.z + RECREATION_ROOM.depth / 2 - 0.4, 0, 0.8, allMeshes)
    );
  }

  // --- Computers as display screens ---
  placedNodes.push(
    placeProp(scene, P.modComputerSmall, 'rr_computer', room,
      RC.x - 4, 0.8, RC.z + 3.5, 0, 0.6, allMeshes)
  );

  // --- Wall vents ---
  placedNodes.push(
    placeProp(scene, P.modVent1, 'rr_vent_1', room,
      RC.x - 4.5, 2.5, RC.z, Math.PI / 2, 1, allMeshes)
  );

  // --- Detail plates on walls ---
  placedNodes.push(
    placeProp(scene, P.modPlateSmall, 'rr_plate_1', room,
      RC.x - 4.5, 1.6, RC.z + 3.5, Math.PI / 2, 0.8, allMeshes)
  );
  placedNodes.push(
    placeProp(scene, P.modPlateSmall, 'rr_plate_2', room,
      RC.x - 4.5, 1.8, RC.z + 3.2, Math.PI / 2, 0.6, allMeshes)
  );

  // --- Lamp ---
  placedNodes.push(
    placeProp(scene, P.lamp1, 'rr_lamp_1', room,
      RC.x, RECREATION_ROOM.height - 0.3, RC.z, 0, 1, allMeshes)
  );
  ...
  addCeilingLight(
    ...
    new Color3(1.0, 0.95, 0.9), 0.45, lights
  );
  addCeilingLight(
    scene, room,
    new Vector3(RC.x + 2, RECREATION_ROOM.height - 0.2, RC.z),
...
async function createScenicCorridor(
  ...
  for (let cz = SC.z + 6; cz >= SC.z - 6; cz -= 4) {
    placedNodes.push(
      placeModel(scene, P.floorCeiling1, `sc_floor_${cz}`, corridor,
        SC.x, -0.05, cz, 0, 1, allMeshes)
    );
  }
  ...
  for (let cz = SC.z + 6; cz >= SC.z - 6; cz -= 4) {
    placedNodes.push(
      placeModel(scene, P.modRoofPlate, `sc_ceil_${cz}`, corridor,
        SC.x, SCENIC_CORRIDOR.height, cz, 0, 1, allMeshes)
    );
  }
  ...
  for (let wz = SC.z + 6; wz >= SC.z - 6; wz -= 4) {
    placedNodes.push(
      placeModel(scene, P.wallSingle, `sc_wall_l_${wz}`, corridor,
        SC.x - SCENIC_CORRIDOR.width / 2, 0, wz, Math.PI / 2, 1, allMeshes)
    );
  }

  // --- Right wall (openings for mess hall and recreation room) ---
  placedNodes.push(
    placeModel(scene, P.wallSingle, 'sc_wall_r_top', corridor,
      SC.x + SCENIC_CORRIDOR.width / 2, 0, SC.z + 6, -Math.PI / 2, 1, allMeshes)
  );
  placedNodes.push(
    placeModel(scene, P.wallSingle, 'sc_wall_r_bot', corridor,
      SC.x + SCENIC_CORRIDOR.width / 2, 0, SC.z - 6, -Math.PI / 2, 1, allMeshes)
  );

  // --- Back wall (entrance from main Corridor A) with doorway ---
  placedNodes.push(
    placeModel(scene, P.doorway, 'sc_door_back', corridor,
      SC.x, 0, SC.z - SCENIC_CORRIDOR.depth / 2, 0, 1, allMeshes)
  );

  // --- Front wall (opening to observation deck) ---
  placedNodes.push(
    placeModel(scene, P.doorway2, 'sc_door_front', corridor,
      SC.x, 0, SC.z + SCENIC_CORRIDOR.depth / 2, 0, 1, allMeshes)
  );

  // --- Pipes along ceiling ---
  placedNodes.push(
    placeProp(scene, P.pipe1, 'sc_pipe_1', corridor,
      SC.x, SCENIC_CORRIDOR.height - 0.15, SC.z, 0, 1, allMeshes)
  );

  // --- Lamps ---
  placedNodes.push(
    placeProp(scene, P.lamp1, 'sc_lamp_1', corridor,
      SC.x, SCENIC_CORRIDOR.height - 0.3, SC.z - 4, 0, 1, allMeshes)
  );
  placedNodes.push(
    placeProp(scene, P.lamp2, 'sc_lamp_2', corridor,
      SC.x, SCENIC_CORRIDOR.height - 0.3, SC.z + 4, 0, 1, allMeshes)
  );

  // --- Wall vents ---
  placedNodes.push(
    placeProp(scene, P.modVent1, 'sc_vent_1', corridor,
      SC.x - SCENIC_CORRIDOR.width / 2 + 0.2, 2.5, SC.z, Math.PI / 2, 0.8, allMeshes)
  );
  ...
  addCeilingLight(
    ...
    new Color3(0.9, 0.95, 1.0), 0.4, lights
  );
  addCeilingLight(
    scene, corridor,
    new Vector3(SC.x, SCENIC_CORRIDOR.height - 0.2, SC.z + 4),
