...
export function createStationEnvironment(scene: Scene): StationEnvironment {
  ...
  // Track extra materials created dynamically for disposal
  const extraMaterials: StandardMaterial[] = [];
  ...
  for (let i = 0; i < 5; i++) {
    ...
    extraMaterials.push(targetMat); // Track for disposal
    ...
    extraMaterials.push(centerMat); // Track for disposal
  ...
  const dispose = () => {
    ...
    // Dispose dynamically created materials
