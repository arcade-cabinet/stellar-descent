...
export class ChunkManager {
  ...
  private generateChunkData(chunkX: number, chunkZ: number): ChunkData {
    ...
    for (let i = 0; i < obstacleCount; i++) {
      // Deterministic randomness for obstacle visuals
      const _obstacleSeed = Math.floor(random() * 1000000);
      obstacles.push({
  ...
  private createObstacleMesh(obstacle: GeneratedObstacle, chunkX: number, chunkZ: number): Mesh {
    ...
    // Use deterministic seed based on chunk and obstacle position
    const obstacleSeed =
      Math.abs(Math.floor(worldX * 73856093 + worldZ * 19349663 + obstacle.scale * 1000)) | 0;
    const random = seededRandom(obstacleSeed);
    ...
    if (savedEntities.length > 0) {
      ...
      for (const saved of savedEntities) {
        ...
        try {
          ...
          console.warn('Failed to parse saved entity data', e);
        }

        const species = ALIEN_SPECIES[speciesId] || ALIEN_SPECIES.skitterer;
        const position = new Vector3(saved.x, saved.y, saved.z);
        const entity = createAlienEntity(this.scene, species, position, Date.now());

        // Restore state
        entity.id = saved.id;
        if (entity.health) {
          entity.health.current = saved.health;
        }
  ...
  unloadChunk(chunkX: number, chunkZ: number): void {
    ...
    for (const entity of chunk.entities) {
      if (entity.transform && entity.health) {
        const data = {
          speciesId: entity.alienInfo?.speciesId,
        };
