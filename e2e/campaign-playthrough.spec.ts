 ...
 */
async function verifyLevelSpecificUI(page: Page, levelId: LevelId): Promise<boolean> {
  const levelIndicators: Record<LevelId, RegExp[]> = {
    anchor_station: [/ANCHOR STATION|PROMETHEUS|Good morning|ATHENA|Sergeant Cole|PREP BAY/i],
    landfall: [/ORBITAL DROP|FREEFALL|DESCENT|ALTITUDE|LANDING|LANDFALL/i],
    fob_delta: [/FOB DELTA|INVESTIGATE|PERIMETER|FLASHLIGHT|SCANNER/i],
    brothers_in_arms: [/BROTHERS|MARCUS|FIRE SUPPORT|REUNION|WAVE|CANYON/i],
    the_breach: [/BREACH|HIVE|QUEEN|SCAN|TUNNEL|CHAMBER/i],
    extraction: [/EXTRACTION|ESCAPE|HOLDOUT|DROPSHIP|LZ|SPRINT/i],
  };
...
test.describe
  .serial('Full Campaign Playthrough', () => {
    test.beforeAll(() => {
    ...
    test('Level 1: Anchor Station - Tutorial', async ({ page }) => {
      ...
      await player.waitForText(/brother's team|FOB Delta|Marcus|Movement/i, 15000).catch(() => {});
    ...
    test('Level 2: Landfall - HALO Drop and Surface Combat', async ({ page }) => {
      test.setTimeout(LEVEL_TIMEOUTS.LONG_PLAYTHROUGH);
    ...
    test('Level 4: Brothers in Arms - Marcus Reunion and Wave Combat', async ({ page }) => {
      test.setTimeout(LEVEL_TIMEOUTS.LONG_PLAYTHROUGH);
    ...
    test('Level 6: Extraction - Escape, Holdout, and Victory', async ({ page }) => {
      ...
      const hasVictory = await player.hasText(/VICTORY|COMPLETE|MISSION|EXTRACTION COMPLETE/i);
    ...
    test('Credits: Verify credits sequence after final level', async ({ page }) => {
      test.setTimeout(LEVEL_TIMEOUTS.MEDIUM_PLAYTHROUGH);
      ...
      const hasVictory = await player.hasText(/MISSION COMPLETE|VICTORY|CAMPAIGN COMPLETE/i);
...
test.describe
  .serial('Save/Load Between Levels', () => {
    test.beforeAll(() => {
      ensureScreenshotsDir();
    });
...
test.describe
  .serial('Game State Persistence', () => {
    ...
    test('Each level has correct chapter assignment in config', async ({ page }) => {
      test.setTimeout(LEVEL_TIMEOUTS.LONG_PLAYTHROUGH);
...
test.describe
  .serial('Level-Specific UI Verification', () => {
    test.beforeAll(() => {
      ensureScreenshotsDir();
    });
...
test.describe('Campaign Flow Regression', () => {
  test('All levels load without console errors', async ({ page }) => {
    ...
    if (criticalErrors.length > 0) {
      console.warn(`Campaign flow had ${criticalErrors.length} critical errors:`, criticalErrors);
    }
  });
